...
export default ({ schedule, task }: Props) => {
    ...
    return (
        <div css={tw`sm:flex items-center p-3 sm:p-6 border-b border-neutral-800`}>
            ...
            <Dialog.Confirm
                open={visible}
                title={'Confirma la exclusión de la tarea'}
                confirm={'Si, excluir tarea'}
            ...
            <div css={tw`flex-none sm:flex-1 w-full sm:w-auto overflow-x-auto`}>
                ...
                {task.payload && (
                    <div css={tw`md:ml-6 mt-2`}>
                        {task.action === 'backup' && (
                            <p css={tw`text-xs uppercase text-neutral-400 mb-1`}>Saltar archivos y carpetas:</p>
                        )}
                        <div
            ...
            <div css={tw`mt-3 sm:mt-0 flex items-center w-full sm:w-auto`}>
                ...
                <Can action={'schedule.update'}>
                    <button
                        type={'button'}
                        aria-label={'Excluir tarea programada'}
